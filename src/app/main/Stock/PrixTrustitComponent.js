...
import {stock} from '../../static';
import swal from 'sweetalert';


function PrixTrustitComponent({currentPrice, _id, locked}) {
       ...
       swal({
            ...
            if (result) {
              fetch(stock.update(_id), {
                  ...
                  body: JSON.stringify({prixTrustit:prix})
              })
                .then(updated => {
                    if (updated) {
                        setPrice(prix);
    ...
    return (
        <div onClick={() => setEdit(true && !locked)}>
            {edit ? 
                ...
                :
                <span>{price == -1 ?  'Après diagnostic' :price==undefined?'0': `${price}`}</span>
            }
        </div>
    )
}

export default PrixTrustitComponent
