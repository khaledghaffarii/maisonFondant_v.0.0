...
const PlanningsSuivi = ({ classes, ...props }) => {
    ...
    const fetchData = async () => {
        return new Promise(async (resolve) => {
            try {
              ...
              if(dataList?.data.listSorties.length!=0){setTestSortie(true);}
...
justifyContent: 'space-between'}}>
                        <Typography style={{padding: '10px 0px 0px 0px'}} component="div" variant="h4">
                            {refPlanning}
                        </Typography>
                            ...
                            <Table aria-label="customized table" >
                                <TableHead>
                                    ...
                                    <StyledTableCell>Code</StyledTableCell>
                                        ...
                                        <StyledTableCell >Etat Livraison</StyledTableCell>
                                        ...
                                        .map((row, index) => (
                                            ...
                                            <StyledTableRow key={row.code}>
                                                ...
                                                <StyledTableCell component="th" scope="row">
                                                    <Barcode format="CODE128" height="15" width="2" fontSize="20
                                                    " fontFamily='"Roboto", "Helvetica", "Arial", sans-serif' value={row.code} />
                                                </StyledTableCell>
                                                <StyledTableCell width="8" fontSize="8" align="left">{row.item}</StyledTableCell>
                                                <StyledTableCell align="left">{row.center_recuperation}</StyledTableCell>
                                                <StyledTableCell align="left">{row.destination}</StyledTableCell>
                                                <StyledTableCell align="right">{row.phone}</StyledTableCell>
                                                <StyledTableCell align="right">{row.prix_client}</StyledTableCell>
                                                <StyledTableCell align="right">{row.status_reparation} </StyledTableCell>
                                                <StyledTableCell align="right">{row.departement}</StyledTableCell>
                                                <StyledTableCell align="left">{row.remarque_CS ? row.remarque_CS : "Rien"}</StyledTableCell>
                                                <StyledTableCell align="right"><StateLivraisonComponent key={row._id} etat={row.staus_livraison} _id={row._id} /></StyledTableCell>
...
export default withTranslation()(PlanningsSuivi);