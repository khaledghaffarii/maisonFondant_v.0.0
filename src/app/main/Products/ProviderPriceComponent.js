...
function ProviderPriceComponent({currentPrice, _id}) {
    const [price, setPrice] = React.useState(currentPrice);
    const updateRep = async (e) => {
        e.preventDefault();
       let prixFournisseur = e.target.value;
       swal({
            title: 'êtes vous sûr?',
            ...
            if (result) {
              fetch(products.update(_id), {
                  ...
                  body: JSON.stringify({prixFournisseur})
              })
                .then(updated => {
                    if (updated) {
                        setPrice(prixFournisseur);
                        setEdit(false);
                    }
                });
            } else {
    ...
    return (
        <div onClick={() => setEdit(true)}>
            {edit ? 
                ...
                :
                <span>{price == -1 ?  'Après diagnostic' : `${price} TND`}</span>
            }
        </div>
    )
}

export default ProviderPriceComponent
