...
function TabsCommandes(props) {
    ...
    return (
        <div>
            ...
            >
                <Tabs.Tab id="tab1" title={`En attente - (${orders.filter(order => order.status == 'En Attente').length})`}>
                    <MaterialTable 
                        title={'Orders'} 
                        style={{width: '80vw'}} 
                        columns={[
                            { title: 'Nom et Prénom', field: '', render: rowData => (<p>{rowData.orderBy.fname} {rowData.orderBy.lname}</p>)},
                            { title: 'Contact', field: '',  render: rowData => (<p>{rowData.orderBy.email} / {rowData.orderBy.phone}</p>)},
                            { title: 'Quantité', field: 'quantity'},
                           
                        ]} 
                        data={orders.filter(order => order.status == 'En Attente')}/>
                </Tabs.Tab>
                <Tabs.Tab id="tab2" title={`Accepté - (${orders.filter(order => order.status == 'Accepté').length})`}>
                    <MaterialTable 
                        title={'Orders'} 
                        style={{width: '80vw'}} 
                        columns={[
                ...
                <Tabs.Tab id="tab3" title={`Rejetée - (${orders.filter(order => order.status == 'Annulé').length})`}>
                <MaterialTable 
                        title={'Orders'} 
                        style={{width: '80vw'}} 
                        columns={[
